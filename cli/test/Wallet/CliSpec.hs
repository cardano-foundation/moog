...
import Wallet.Cli
    ...
    , WalletError (WalletAlreadyDecrypted, WalletPresent)
    , WalletInfo (..)
    , walletCmd
    )
...
import Submitting qualified

tryObtainingDecryptedWallet
    :: Text -> Either Submitting.WalletError Wallet
tryObtainingDecryptedWallet mnemonicText =
    let mnemonic = ClearText mnemonicText
    in  readWallet (False, mnemonic)

tryRetrieveCreatedWallet
    :: FilePath -> IO (Either Submitting.WalletError Wallet)
...
getDecryptedWalletForTesting = do
    ...
    fromRight
        (error "after above check wallet is sure to be properly formed")
        decryptedWalE
...
spec = do
    ...
    describe "wallet encrypt/decrypt" $ do
        ...
        it
            "wallet cannot be created from invalid mnemonic - wrong number of words"
            $ do
                let mnemonicText =
                        "culture island clump online fatigue curve fish mandate echo cradle cat arrange upset region"
                    decryptedWal = tryObtainingDecryptedWallet mnemonicText
                decryptedWal `shouldSatisfy` isLeft
        it
            "wallet cannot be created from invalid mnemonic - proper number of words, not all legal"
            $ do
                let mnemonicText =
                        "culture island clump online fatigue curve fish mandate echo cradle cat arrange upset regions"
                    decryptedWal = tryObtainingDecryptedWallet mnemonicText
                decryptedWal `shouldSatisfy` isLeft
        ...
        it
            "wallet cannot be decrypted when created previously in decrypted state"
            $ do
                withSystemTempDirectory "wallet-cli-spec" $ \dir -> do
                    let walletDir = dir <> "/wallet"
        ...
        it
            "wallet cannot be encrypted when created previously in decrypted state if the same filepath is chosen"
            $ do
                withSystemTempDirectory "wallet-cli-spec" $ \dir -> do
                    let walletDir = dir <> "/wallet"
                        commandCreate = Create walletDir (Nothing :: Maybe Text)
        ...
        it
            "wallet can be encrypted when created previously in decrypted state when other filepath is chosen"
            $ do
                withSystemTempDirectory "wallet-cli-spec" $ \dir -> do
                    let walletDir = dir <> "/wallet"
                        commandCreate = Create walletDir (Nothing :: Maybe Text)
                    ...
                    let decryptedWal =
                            fromRight
                                (error "after above check wallet is sure to be properly formed")
                                decryptedWalE
                        walletDirOther = dir <> "/wallet1"
                    encryptedWal <-
                        walletCmd $ Encrypt decryptedWal "password" walletDirOther
                    encryptedWal `shouldSatisfy` isRight
